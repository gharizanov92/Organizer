...
def do_register():
    ...
    if db.users.find_one({"username":username}):
        ...
        db.users.save({"_id":str(ObjectId()),"username":username, "password":password})
...
def manager():
    ...
    total = db.reports.aggregate([{"$match":{"day":now.day}},{"$group":{  "_id":"$day", "total": {"$sum": "$amount"}}}])
    ...
    total = db.reports.aggregate([{"$match":{"month":now.month}},{"$group":{  "_id":"$month", "total": {"$sum": "$amount"}}}])
